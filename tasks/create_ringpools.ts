...
import { getPoolnames_off_line } from "../lib/pool";
import { getwhitelistPath, initialzeAssets, loadJson, arrayTemplate,getJsonPath ,
loadWhiteList,saveWhiteList} from "../lib/whitelist"
...
task(async ({ wallets, client, refs }) => {
  ...
  //
  // generate factory address array
  ...
  for (let index = 0; index < ringpools[networkname].length; index++) {
    ...
    let x_whitelist = loadWhiteList(_whitelistPath)
    if (x_whitelist.pair[element]) {
    ...
    // start to make pair
    //
    x_whitelist = loadWhiteList(_whitelistPath)
    const msgExecuteContract = new MsgExecuteContract(wallets[signer].key.accAddress,
      ...
      new Coins([]))
    //
    console.log(msgExecuteContract.execute_msg['create_pair']['asset_infos'])
    //
